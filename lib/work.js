...
var requestHandler = null; // require('./request_handler');

var Work  = function(router) {   
    //server references router to handle request
    this.router = new Router(requestHandler);
...
Work.prototype.define = function(route, httpMethod, callback) {
    if (!httpMethod) {
        ...
        this.router[httpMethod][route] = callback;
    }
};
...
Work.prototype.listen = function(port, callback) {
    ...
    this.server = http.createServer(function(req, res) {
        ...
        if (that.router[req.method][req.url]) that.router[req.method][req.url](req, res);  
